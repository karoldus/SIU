...
class TurtlesimEnvBase(metaclass=abc.ABCMeta):
    ...
    def setup(
        self, routes_fname: str, agent_cnt=None  # nazwa pliku z definicją scenariuszy
    ):  # ograniczenie na liczbę tworzonych agentów (None - brak ogr., agenty wg scenariuszy)
    ...
    def reset(
        self, tnames=None, sections="default"  # lista nazw zółwi do resetu (None=wszystkie)
    ) -> dict:  # nry sekcji trasy dla każdego żółwia (default=domyślny,random=losowy)
        self.step_sum = 0
        if tnames is None:
        ...
        for tidx, tname in enumerate(tnames):
            ...
            while True:
                ...
                if abs(fx) + abs(fy) > 0.01:  # w obrębie drogi
                    theta += np.random.uniform(
                        -np.pi / self.PI_BY, np.pi / self.PI_BY
    ...
    @abc.abstractmethod
    def step(
        self, actions: dict, realtime=False  # działania żółwi, actions={tname:(speed,turn)}
    ) -> dict:  # flaga sterowania prędkościowego
        pass
    ...
    def get_road(self, tname):
        ...
        fx = 0.02 * (color.r - 200)  # składowa x zalecanej prędkości <-1;1>
        fy = 0.02 * (color.b - 200)  # składowa y zalecanej prędkości <-1;1>
